...
Dependency provenance is recorded in [`thirdparty/dependencies.json`](thirdparty/dependencies.json). The offline gate verifies immutable source revisions, licenses, populated CPM caches, and the Tree-sitter/WebEngine feature matrix while emitting `dependency-inventory.json` and an SPDX JSON SBOM. Run `python scripts/dependency_inventory.py --check --tree-sitter on --webengine off`; maintenance runs can add `--check-updates`, or use `--refresh-advisories --security-gate` to query OSV without credentials.

Editor language services are available when their server is installed on `PATH`: `clangd` for C/C++, `pyright-langserver` for Python, `rust-analyzer` for Rust, and `gopls` for Go. Servers are shared by detected workspace root and compatible server configuration, and workspace folders are advertised during initialization. Diagnostics are version-gated, hover/definition requests are cancellable and time-bounded, and stale replies cannot replace a newer buffer state. File buffers use `file:` URIs; unsaved buffers use an explicit `untitled:` URI until saved, and non-local definition targets are reported instead of opened. Diagnostics are shown as editor squiggles, hover information appears after briefly resting the pointer over a symbol, and `F12` requests go-to-definition. The status bar reports server startup, initialization, stop, crash, and request errors; the editor continues to work normally when a server is not installed.
...
Workspace Trust keeps JavaScript execution and script-file access, JavaScript document edit/save/open operations, the integrated or external terminal, Lua console execution, and an optional `.notepadnext/startup.lua` independently denied until explicitly allowed. **Settings > Workspace Trust** shows the current workspace, supports Allow Once/Always Allow grants, and revokes each capability separately; the audit trail stores only a hashed workspace identity plus operation and result. Bundled resources remain trusted, while native plugin decisions remain under **Plugins > Plugin Trust**.
...
Text files from 50 MiB through the 128 MiB safety limit open in read-only large-file mode. Whole-document highlighting, minimap, sticky scroll, autocomplete, Git decorations, LSP, schema validation, and Markdown preview are disabled or limited to the visible range, with the status bar explaining the policy. Larger text files are rejected with a clear prompt to use the Hex Editor or an external tool.
...
**File > Open Remote File...** opens an SFTP document through the pinned libssh2 client. Password and private-key authentication are supported; credentials stay in memory, the document is mirrored in a temporary local file for editing, and Save checks the remote size/mtime before uploading to a same-directory temporary path and atomically replacing the destination. Concurrent remote edits produce a conflict instead of overwriting silently; servers without the required atomic-rename capability are rejected without changing the destination. Known host keys are checked against an OpenSSH `known_hosts` file, with an explicit fingerprint confirmation for a new host.
